...
void vTaskGUI(void *arg)
{
    ...
    lv_color_t *buf1 = (lv_color_t *)(heap_caps_malloc(LV_HOR_RES_MAX * LV_VER_RES_MAX * sizeof(lv_color_t), MALLOC_CAP_DMA));
    lv_color_t *buf2 = (lv_color_t *)(heap_caps_malloc(LV_HOR_RES_MAX * LV_VER_RES_MAX * sizeof(lv_color_t), MALLOC_CAP_DMA));
...
void vTaskGUIUpdate(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(xGuiUpdateQueue, &received_data, portMAX_DELAY) == pdTRUE)
        {
            if (xSemaphoreTake(xGuiSemaphore, portMAX_DELAY == pdTRUE))
            {
                char buf[64];
